...
class F1_selection(F1_abc):

    @overrides
    def __call__(self, predictions: List[List[int]], gold_labels: List[List[int]]):
        for g, p in zip(gold_labels, predictions):
            g_set = set([i  for i in range(len(g)) if g[i] == 1])
            p_set = set([i  for i in range(len(p)) if p[i] == 1])
            self.A += len(g_set & p_set)
            self.B += len(p_set)
            self.C += len(g_set)